...
class Encoder(nn.Module):#variational encoder
    def __init__(self, latent_dims):
        ...
        self.a = torch.nn.Parameter(torch.ones(3500))
...
def train(autoencoder,data, indexData,epochs=2):
    lam = 1 #parameter lambda
    opt1 = torch.optim.Adam(autoencoder.parameters(),lr=0.000004)
    vae_loss = []
    lin_loss = []
    estimatedindex = []
    actualindex = []
    ...
    for epoch in range(epochs):
        ...
        for x,y1 in zip(data,indexData): #range data to access both this should def. be batched n done using sgd
            ...
            z = torch.mean(autoencoder.latentForward(x))
            estimatedindex.append(z.detach().numpy())
            actualindex.append(y1.detach().numpy())
            loss1 = 2000*((y1-z)**2) + 0.01*(0.5*((x - x_hat)**2).mean() + 3*autoencoder.encoder.kl)
            print("L")
            print(loss1)
            #print(z)
            print(y1)
...
model = train(autoencoder, data, indexData)


torch.save(autoencoder.state_dict(),"indexVAE.pth")
